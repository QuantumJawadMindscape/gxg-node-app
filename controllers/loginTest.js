...
export async function testLogin(req, res) {
    ...
    try {
        ...
        const clientUrl = `https://apis.gxg.app/BOWCF/Backoffice.svc/CreateClient?SessionID=${sessionID}&FirstName=TestUser&Username=${clientUsername}&Password=Test@1234&InvestorPassword=Invest@1234&Country=Pakistan&ParentID=8564&callback=handleCreateClient`;
        ...
        let clientId = clientJson?.ClientID || clientJson?.data?.ClientID || clientJson?.Client_Id || null;
        if (!clientId && typeof clientJson === "string") clientId = parseInt(clientJson.replace(/"/g, "").trim()) || null;
        if (!clientId) clientId = parseInt(clientRaw.replace(/[^0-9]/g, "").trim()) || null;
        ...
        if (!accountId && typeof accJson === "string") accountId = parseInt(accJson.replace(/"/g, "").trim()) || null;
        if (!accountId) accountId = parseInt(accRaw.replace(/[^0-9]/g, "").trim()) || null;
